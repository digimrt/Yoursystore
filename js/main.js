...
function displaySearchResults(results, input) {
  ...
  if (!dropdown) {
    ...
    dropdown.className =
      "absolute top-full mt-2 w-full bg-background border border-border rounded-lg shadow-lg z-50 max-h-96 overflow-y-auto";
...
function renderProductCard(product) {
  ...
  if (hasDiscount) {
    displayPrice = Math.round(product.price * (1 - discountPercent / 100));
        ...
        <div class="product-card group bg-card border border-border/50 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:border-accent/30" data-product-id="${product.id}">
            ...
            <div class="p-4">
                <a href="product.html?id=${product.id}" class="block">
                    ...
                    ${product.subtitle ? `<p class="text-xs text-muted-foreground text-center mb-2">${getProductSubtitle(product)}</p>` : ""}
                </a>
                <div class="flex items-center justify-center gap-2 mb-3">
                    <span class="text-accent font-bold text-lg">${formatPrice(displayPrice)}</span>
                    ${hasDiscount ? `<span class="text-muted-foreground line-through text-sm">${formatPrice(originalPrice)}</span>` : ""}
                </div>
...
function renderProducts() {
  ...
  let products = currentSearchQuery
  ...
  if (scrollToId) {
    setTimeout(() => {
      const targetProduct = document.querySelector(
        `.product-card[data-product-id="${scrollToId}"]`,
      );
      if (targetProduct) {
...
function renderCartDrawer() {
  ...
  content.innerHTML = `<div class="space-y-4 mb-6">${items
    .map(
      (item) => `
...
function renderCartPage() {
  ...
  container.innerHTML = `<div class="space-y-4">${items
    .map(
      (item) => `
...
function updateCheckoutButton() {
  ...
  else if (selectedDeliveryOption === "delivery" && !address)
    btn.disabled = true;
...
function handleContactSubmit(event) {
  ...
  let whatsappMessage = "";
  if (lang === "ar")
    whatsappMessage = `رسالة جديدة من موقع متجر موري:%0A%0Aالاسم: ${name}%0Aالبريد: ${emailText}%0Aالهاتف: ${phone}%0A%0Aالرسالة:%0A${message}`;
  else if (lang === "fr")
    whatsappMessage = `Nouveau message du site Mauri Store:%0A%0ANom: ${name}%0AEmail: ${emailText}%0ATéléphone: ${phone}%0A%0AMessage:%0A${message}`;
  else
    whatsappMessage = `New message from Mauri Store website:%0A%0AName: ${name}%0AEmail: ${emailText}%0APhone: ${phone}%0A%0AMessage:%0A${message}`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (document.getElementById("cart-items-container")) {
    ...
    if (addressField)
      addressField.addEventListener("input", updateCheckoutButton);
  }
